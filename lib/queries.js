...
var writeGoal = (uname, duration, goal, callback) =>
{
	var in_progress = true;
	var sql = fs.readFileSync(path.join(__dirname + '/sql/new_goal.sql')).toString();

	db.none(sql, [uname, duration, in_progress, goal])
	.catch((error) => {
		console.log(error);
		return callback(false);
	});
...
{
	var sql = fs.readFileSync(path.join(__dirname + '/sql/update_goal_status')).toString();
	console.log("in-progress = " + status);

	db.none(sql, [uname, created_at, status])
	.catch((error) => {
		console.log(error);
		return callback(false);
	});
